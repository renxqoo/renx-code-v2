...
export async function setWriteBufferSessionTargetPath(
  session: Pick<WriteBufferSessionMeta, 'metaPath'>,
  targetPath: string
): Promise<void> {
  const normalizedTargetPath = path.resolve(targetPath);
  const meta = await readMeta(session.metaPath);
  if (meta.targetPath === normalizedTargetPath) {
    return;
  }

  meta.targetPath = normalizedTargetPath;
