...
Rules:
- Focus on concrete implementation details that matter for the next coding step.
- Preserve important file paths, function names, interfaces, config keys, and test expectations.
- Record failures, edge cases, and explicit user preferences.
- Keep the summary compact but specific.
- Do not include <analysis>.
- Do not add commentary outside the <summary> block.`;

export const COMPACTION_SYSTEM_PROMPT = COMPACTION_SYSTEM_PROMPT_V1;

export function resolveCompactionSystemPrompt(version: CompactionPromptVersion = 'v1'): string {
  return version === 'v2' ? COMPACTION_SYSTEM_PROMPT_V2 : COMPACTION_SYSTEM_PROMPT_V1;
}
