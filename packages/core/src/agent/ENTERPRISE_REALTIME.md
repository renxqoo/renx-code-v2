...
interface LLMProvider {
  ...
  // 生成响应 (流式)
  generateStream(messages: Message[], config?: LLMConfig): AsyncGenerator<Chunk>;
}
...
class RealTimeStorage {
  ...
  async getMessagesAfterCheckpoint(
    ...
    const allMessages = await this.redis.lrange(`conversation:${conversationId}:messages`, 0, -1);
