...
function locateRipgrep(
  customPath?: string
): { found: true; command: string; argsPrefix: string[] } | { found: false; error: string } {
  ...
  for (const candidate of uniqueCandidates) {
    const isScriptWrapper = /\.(cjs|mjs|js)$/i.test(candidate.bin);
    const command = isScriptWrapper ? process.execPath : candidate.bin;
    const argsPrefix = isScriptWrapper ? [candidate.bin] : [];
    const probe = spawnSync(command, [...argsPrefix, '--version'], { stdio: 'ignore' });
    if (!probe.error && probe.status === 0) {
      return { found: true, command, argsPrefix };
    }
  }
  ...
  async execute(args: GrepArgs, context?: ToolExecutionContext): Promise<ToolResult> {
    try {
      ...
      const result = await this.runRipgrep(
        located.command,
        [...located.argsPrefix, ...commandArgs],
        rootPath,
        args,
        context
      );
