...
export class TaskStopTool extends BaseTool<typeof schema> {
  ...
  async execute(args: TaskStopArgs): Promise<ToolResult> {
    ...
    // Stop should not preflight with poll(), because some runner implementations
    // can advance or finalize a run as a side effect of polling.
    const cancelled = await this.runner.cancel(normalizedNamespace, agentId, args.reason);
    if (!cancelled) {
      const current = await this.runner.poll(normalizedNamespace, agentId);
      if (!current) {
        return buildTaskFailure('AGENT_RUN_NOT_FOUND', `agent run not found: ${agentId}`, {
          namespace: normalizedNamespace,
        });
      }
      if (isAgentRunTerminal(current.status)) {
        return buildTaskFailure(
          'AGENT_RUN_ALREADY_TERMINAL',
          `agent run already terminal: ${current.status}`,
          {
            namespace: normalizedNamespace,
            agent_id: agentId,
          }
    ...
    if (isAgentRunTerminal(cancelled.status) && cancelled.status !== 'cancelled') {
      return buildTaskFailure(
        'AGENT_RUN_ALREADY_TERMINAL',
        `agent run already terminal: ${cancelled.status}`,
        {
          namespace: normalizedNamespace,
          agent_id: agentId,
        }
