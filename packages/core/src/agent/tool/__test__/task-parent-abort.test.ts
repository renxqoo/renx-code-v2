...
describe('task-parent-abort cascade', () => {
  ...
  it('still cancels linked task when runner cancel returns null', async () => {
    await store.updateState('n3b', (state) => {
      state.tasks.t_active = makeTask('t_active', 'in_progress', 'agent-null');
      return null;
    });
