...
describe('task_stop edge branches', () => {
  ...
  it('treats cancel returning a non-cancelled terminal run as already terminal', async () => {
    const runner: SubagentRunnerAdapter = {
      start: async () => makeRun(),
      poll: async () => makeRun({ status: 'running', agentId: 'agent-terminal-after-cancel' }),
      cancel: async () =>
        makeRun({
          status: 'completed',
          agentId: 'agent-terminal-after-cancel',
          endedAt: Date.now(),
        }),
    };
    const stopTool = new TaskStopTool({ store, runner });
    ...
    const result = await stopTool.execute({
      namespace: 'n4',
      agent_id: 'agent-terminal-after-cancel',
    });

    expect(result.success).toBe(false);
    expect(result.output).toContain('AGENT_RUN_ALREADY_TERMINAL');
