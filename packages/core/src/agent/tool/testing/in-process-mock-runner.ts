...
export class InProcessMockRunnerAdapter implements SubagentRunnerAdapter {
  ...
  async poll(namespace: string, agentId: string): Promise<AgentRunEntity | null> {
    ...
    const reachedScheduledCompletion =
      run.status === 'running' &&
      typeof startedAt === 'number' &&
      this.now() - startedAt >= this.completionDelayMs;

    if (reachedScheduledCompletion) {
      const completed = await this.completeRun(namespace, agentId);
      if (completed) {
        return completed;
      }
    }
