...
describe('ProviderRegistry', () => {
  beforeEach(() => {
    ...
    delete process.env.KIMI_API_BASE;
  ...
  describe('MODEL_CONFIGS', () => {
    it('should have all model configurations', () => {
      ...
      const modelConfigs = MODEL_CONFIGS as Record<string, { id: string }>;

      modelIds.forEach((id) => {
        expect(modelConfigs[id]).toBeDefined();
        expect(modelConfigs[id]?.id).toBe(id);
      });
    });
  ...
  describe('listModelsByProvider', () => {
    ...
    it('should return models for minimax provider', () => {
      ...
      expect(models).toHaveLength(2);
      expect(models.map((m) => m.id)).toContain('minimax-2.5');
      expect(models.map((m) => m.id)).toContain('minimax-2.7');
    });
