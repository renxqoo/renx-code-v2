...
describe('buildReplyRenderItems', () => {
  it('groups tool segments by call id and keeps non-tool segments in place', () => {
    const segments: ReplySegment[] = [
      { id: '1:thinking:1', type: 'thinking', content: 'thinking' },
      {
        ...
        type: 'code',
        content: '# Tool: local_shell (call_a)\n$ echo a\n',
        data: {
          id: 'call_a',
          function: { name: 'local_shell', arguments: '{"command":"echo a"}' },
        },
      },
      { id: '1:tool:call_a:stdout', type: 'code', content: 'a\n' },
      {
        ...
        type: 'code',
        content: '# Result: local_shell (call_a) success\na\n',
  ...
  it('starts a new tool group when call id changes', () => {
    const segments: ReplySegment[] = [
      ...
      {
        id: '1:tool-use:call_b',
        type: 'code',
        content: '# Tool: local_shell (call_b)\n$ echo b\n',
      },
      {
        id: '1:tool-result:call_b',
        type: 'code',
        content: '# Result: local_shell (call_b) success\nb\n',
      },
    ];
