...
import { parseInboundMessage, restoreContextTokens, setContextToken, } from "./messaging/inbound";
 ...
 */
export async function startWeixinMonitor(opts) {
    const { baseUrl, token, accountId, abortSignal, longPollTimeoutMs, onMessage, } = opts;
    ...
    while (!abortSignal?.aborted) {
        try {
            ...
            aLog.debug(`getUpdates response: ret=${resp.ret}, msgs=${resp.msgs?.length ?? 0}`);
